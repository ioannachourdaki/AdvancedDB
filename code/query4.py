...
query4a1 = """
SELECT YEAR(c.`DATE OCC`) AS Year,  ROUND(AVG(get_distance(c.LAT, c.LON, p.Y, p.X)), 3) AS `Average Distance`, COUNT(*) AS Count
...
query4b1 = """
SELECT p.DIVISION AS Division,  ROUND(AVG(get_distance(c.LAT, c.LON, p.Y, p.X)), 3) AS `Average Distance`, COUNT(*) AS Count
...
query4a2 = """
SELECT Year,  ROUND(AVG(`Min Distance`), 3) AS `Average Distance`, COUNT(*) AS Count
FROM (
...
query4b2 = """
SELECT t1.DIVISION AS Division,  ROUND(AVG(t2.Min_Distance), 3) AS `Average Distance`, COUNT(*) AS Count
FROM (
...
INNER JOIN (
        SELECT c.DR_NO, MIN(get_distance(c.LAT, c.LON, p.Y, p.X)) AS Min_Distance
        FROM CrimeView2 AS c
        CROSS JOIN Precincts p
        GROUP BY DR_NO
    ) AS t2 ON t1.DR_NO = t2.DR_NO AND t1.Distance = t2.Min_Distance
GROUP BY Division
ORDER BY Count DESC;
"""
